...
# ---------- pane helpers ----------
cmux_find() {  # echoes "window workspace pane surface" if a surface titled $label exists
...
for line in sys.stdin:
    if "surface " in line:
        ...
        if m and m.group(2) == target:
            print(window or "", workspace or "", pane or "", m.group(1)); sys.exit(0)
        continue
    if "pane " in line:
...
EOF
  ...
  cmux send --surface "$sfc" "cd $wt" >/dev/null
  cmux send-key --surface "$sfc" Enter >/dev/null
